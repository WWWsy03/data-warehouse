...
import { queryByTimeYearMonth } from '@/services/data-warehouse/api'

type QueryParam = { year?: number; month?: number }
...
export default function ByTimeQuery() {
  ...
  const query = async () => {
    ...
    if (data === undefined || data?.year === undefined || data?.month === undefined) {
      message.warning('有必填字段未填')
      return
    }
    const resp = await queryByTimeYearMonth(data.year, data.month)
    if (resp.success) {
      setYear(data.year)
      setMonth(data.month)
      setQueryResult(resp)
          ...
          <>
            ...
            <QueryResult result={queryResult}>
              <div className={'flex flex-col gap-2 h-full items-center justify-center'}>
                ...
                共查询到{queryResult.data}部在{year}年{month}月的影片
              </div>
            </QueryResult>
