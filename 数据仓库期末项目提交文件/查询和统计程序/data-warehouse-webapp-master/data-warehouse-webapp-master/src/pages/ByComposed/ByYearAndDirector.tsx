...
import { queryByYearAndDirector, queryByYearAndType } from '@/services/data-warehouse/api'

type QueryParam = { year: number; director: string }
...
export default function ByYearAndType() {
  ...
  const query = async () => {
    ...
    if (data === undefined || data?.year === undefined || data?.director === undefined) {
      message.warning('有必填字段未填')
      return
    }
    const resp = await queryByYearAndDirector(data.year, data.director)
    if (resp.success) {
      setQueryResult(resp)
      setQueryParam(data)
    }
        ...
        <Card>
          <ProForm<QueryParam> formRef={formRef} onFinish={query} autoFocusFirstInput>
            <ProForm.Group title={'查询参数'}>
              ...
              <ProFormText required name="director" label="导演" />
            </ProForm.Group>
          </ProForm>
        </Card>
          ...
          <>
            ...
            <QueryResult result={queryResult}>
              <div className={'flex flex-col gap-2 h-full items-center justify-center'}>
                ...
                共查询到{queryResult.data}部在{queryParam?.year}年上映、由{queryParam?.director}执导的影片
              </div>
            </QueryResult>
