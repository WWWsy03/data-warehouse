...
import { queryByDirectorName } from '@/services/data-warehouse/api'

type QueryParam = { name: string }

export default function ByDirector() {
  ...
  const query = async () => {
    ...
    const resp = await queryByDirectorName(data.name)
    if (resp.success) {
      setQueryResult(resp)
    }
  }
        ...
        <Card>
          <ProForm<QueryParam> formRef={formRef} onFinish={query} autoFocusFirstInput>
            <ProForm.Group title={'查询参数'}>
              <ProFormText required name="name" label="导演名" />
            </ProForm.Group>
          </ProForm>
        </Card>
